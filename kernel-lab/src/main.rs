...
fn print_kernel_uptime_status() {
    use core::fmt::Write;

    let uptime = kernel_clock_status_snapshot();
    let mut vga_writer = vga::VgaWriter;
    let mut serial_writer = serial::SerialWriter;
    let _ = write!(
        vga_writer,
        "KERNEL UPTIME\nsource: {}\nunit: ticks\nruntime: {}\nticks: {:04}\n",
        uptime.source, uptime.runtime, uptime.ticks
    );
    let _ = write!(
        serial_writer,
        "KERNEL UPTIME\nsource: {}\nunit: ticks\nruntime: {}\nticks: {:04}\n",
                            ...
                            if c == '\x08' {
                                ...
                                if LINE_LEN > 0 {
                                        ...
                                        core::str::from_utf8(&LINE_BUFFER[..LINE_LEN])
                                    {
                                        ...
                                        } else if line_str == "kernel-uptime-status" {
                                            print_kernel_uptime_status();
