...
fn gfx_console_input_scancode_to_ascii(scancode: u8) -> Option<u8> {
    match scancode {
        0x1E => Some(b'a'),
        0x30 => Some(b'b'),
        0x2E => Some(b'c'),
...
fn read_gfx_console_input_line(input: &mut [u8; GFX_CONSOLE_INPUT_CAPACITY]) -> usize {
...
fn run_gfx_console_input_echo() {
    let mut input: [u8; GFX_CONSOLE_INPUT_CAPACITY] = [0; GFX_CONSOLE_INPUT_CAPACITY];
    let input_len = read_gfx_console_input_line(&mut input);
    serial::print("gfx-console-input: captured line: ");
    if let Ok(input_text) = core::str::from_utf8(&input[..input_len]) {
...
fn print_gfx_console_shell_app_dispatched(app_name: &[u8]) {
    serial::print("gfx-console-shell: app dispatched: ");
...
fn print_gfx_console_shell_app_not_found(app_name: &[u8]) {
    serial::print("gfx-console-shell: app not found: ");
    if let Ok(input_text) = core::str::from_utf8(app_name) {
        let mut writer = serial::SerialWriter;
        let _ = write!(writer, "{}\n", input_text);
    } else {
...
fn irq0_window_status_snapshot() -> pic::Irq0WindowStatus {
    let remap = pic::ProgrammableInterruptController::pic_remap_state();
    let eoi = eoi_write_hw_smoke_status_snapshot();
    let bind = irq0_bind_hw_smoke_status_snapshot();
    let unmask = irq0_unmask_hw_smoke_status_snapshot();
    pic::ProgrammableInterruptController::irq0_window_status(
        remap.executed,
        eoi.manual_pic_eoi_smoke_proven_this_boot == "yes",
        bind.irq0_bind_proven_this_boot == "yes",
        unmask.irq0_unmask_proven_this_boot == "yes",
    )
}

fn irq0_window_arm_snapshot() -> pic::Irq0WindowStatus {
    let remap = pic::ProgrammableInterruptController::pic_remap_state();
    let eoi = eoi_write_hw_smoke_status_snapshot();
    let bind = irq0_bind_hw_smoke_status_snapshot();
    let unmask = irq0_unmask_hw_smoke_status_snapshot();
    pic::ProgrammableInterruptController::irq0_window_arm(
        remap.executed,
        eoi.manual_pic_eoi_smoke_proven_this_boot == "yes",
        bind.irq0_bind_proven_this_boot == "yes",
        unmask.irq0_unmask_proven_this_boot == "yes",
...
fn irq0_window_fire_snapshot() -> pic::Irq0WindowStatus {
    pic::ProgrammableInterruptController::irq0_window_fire()
}

fn irq0_window_clear_snapshot() -> pic::Irq0WindowStatus {
    pic::ProgrammableInterruptController::irq0_window_clear()
}

fn irq0_ticks_status_snapshot() -> pic::Irq0TicksStatus {
    let remap = pic::ProgrammableInterruptController::pic_remap_state();
    let eoi = eoi_write_hw_smoke_status_snapshot();
    let bind = irq0_bind_hw_smoke_status_snapshot();
    let unmask = irq0_unmask_hw_smoke_status_snapshot();
    pic::ProgrammableInterruptController::irq0_ticks_status(
        remap.executed,
        eoi.manual_pic_eoi_smoke_proven_this_boot == "yes",
        bind.irq0_bind_proven_this_boot == "yes",
        unmask.irq0_unmask_proven_this_boot == "yes",
...
fn irq0_ticks_arm_snapshot() -> pic::Irq0TicksStatus {
    let remap = pic::ProgrammableInterruptController::pic_remap_state();
    let eoi = eoi_write_hw_smoke_status_snapshot();
    let bind = irq0_bind_hw_smoke_status_snapshot();
    let unmask = irq0_unmask_hw_smoke_status_snapshot();
    pic::ProgrammableInterruptController::irq0_ticks_arm(
        remap.executed,
        eoi.manual_pic_eoi_smoke_proven_this_boot == "yes",
        bind.irq0_bind_proven_this_boot == "yes",
        unmask.irq0_unmask_proven_this_boot == "yes",
...
fn irq0_ticks_fire_snapshot() -> pic::Irq0TicksStatus {
    pic::ProgrammableInterruptController::irq0_ticks_fire()
}

fn irq0_ticks_clear_snapshot() -> pic::Irq0TicksStatus {
    pic::ProgrammableInterruptController::irq0_ticks_clear()
}

fn irq0_runtime_status_snapshot() -> pic::Irq0RuntimeStatus {
    let remap = pic::ProgrammableInterruptController::pic_remap_state();
    let eoi = eoi_write_hw_smoke_status_snapshot();
    let bind = irq0_bind_hw_smoke_status_snapshot();
    let unmask = irq0_unmask_hw_smoke_status_snapshot();
    pic::ProgrammableInterruptController::irq0_runtime_status(
        remap.executed,
        eoi.manual_pic_eoi_smoke_proven_this_boot == "yes",
        bind.irq0_bind_proven_this_boot == "yes",
        unmask.irq0_unmask_proven_this_boot == "yes",
...
fn irq0_runtime_start_snapshot() -> pic::Irq0RuntimeStatus {
    let remap = pic::ProgrammableInterruptController::pic_remap_state();
    let eoi = eoi_write_hw_smoke_status_snapshot();
    let bind = irq0_bind_hw_smoke_status_snapshot();
    let unmask = irq0_unmask_hw_smoke_status_snapshot();
    pic::ProgrammableInterruptController::irq0_runtime_start(
        remap.executed,
        eoi.manual_pic_eoi_smoke_proven_this_boot == "yes",
        bind.irq0_bind_proven_this_boot == "yes",
        unmask.irq0_unmask_proven_this_boot == "yes",
...
fn irq0_runtime_stop_snapshot() -> pic::Irq0RuntimeStatus {
    let remap = pic::ProgrammableInterruptController::pic_remap_state();
    let eoi = eoi_write_hw_smoke_status_snapshot();
    let bind = irq0_bind_hw_smoke_status_snapshot();
    let unmask = irq0_unmask_hw_smoke_status_snapshot();
    pic::ProgrammableInterruptController::irq0_runtime_stop(
        remap.executed,
        eoi.manual_pic_eoi_smoke_proven_this_boot == "yes",
        bind.irq0_bind_proven_this_boot == "yes",
        unmask.irq0_unmask_proven_this_boot == "yes",
...
struct KernelClockSnapshot {
    source: &'static str,
    runtime: &'static str,
    ticks: u32,
    irq0_masked: &'static str,
    sti_enabled: &'static str,
}

fn kernel_clock_status_snapshot() -> KernelClockSnapshot {
    let runtime = irq0_runtime_status_snapshot();
    KernelClockSnapshot {
        source: "IRQ0 runtime",
        runtime: if runtime.state == "RUNNING" {
            "running"
        } else {
            "stopped"
        },
        ticks: if runtime.saved_original_master_mask_valid == "yes" {
            runtime.ticks
        } else {
            8
        },
        irq0_masked: runtime.irq0_currently_masked,
        sti_enabled: runtime.sti_currently_enabled,
    }
}
...
fn print_irq0_window_status() {
    use core::fmt::Write;

    let window = irq0_window_status_snapshot();
    let mut serial_writer = serial::SerialWriter;
    let _ = write!(serial_writer, "IRQ0 delivery one-shot window\nstate: {}\nIRQ0 deliveries: {}\nIRQ0 currently masked: {}\nSTI currently enabled: {}\noriginal PIC mask restored: {}\nIF disabled before return: {}\nruntime irq active: {}\npreconditions:\n- PIC remap proof: {}\n- manual PIC_EOI proof: {}\n- IRQ0 descriptor bind proof: {}\n- transactional IRQ0 unmask proof: {}\nunmet preconditions: {}\n",
        window.state,
        window.irq0_deliveries,
        window.irq0_currently_masked,
        window.sti_currently_enabled,
        window.original_pic_mask_restored,
        window.if_disabled_before_return,
        window.runtime_irq_active,
        window.pic_remap_proof,
        window.manual_pic_eoi_proof,
        window.irq0_descriptor_bind_proof,
        window.transactional_irq0_unmask_proof,
        window.unmet_preconditions
    );
}

fn print_irq0_window_arm() {
    use core::fmt::Write;

    let window = irq0_window_arm_snapshot();
    let mut serial_writer = serial::SerialWriter;
    let _ = write!(serial_writer, "IRQ0 delivery one-shot window arm\n{}\nstate: {}\narmed: {}\nIRQ0 deliveries: {}\nhardware mutation: {}\nruntime irq active: {}\nunmet preconditions: {}\n",
        window.result,
        window.state,
        window.armed,
        window.irq0_deliveries,
        window.hardware_mutation,
        window.runtime_irq_active,
        window.unmet_preconditions
    );
}

fn print_irq0_window_fire() {
    use core::fmt::Write;

    let window = irq0_window_fire_snapshot();
    vga_window::draw_irq0_status(window.vga_irq0_status);
    let mut serial_writer = serial::SerialWriter;
    let _ = write!(serial_writer, "IRQ0 delivery one-shot window fire\n{}\nstate: {}\nIRQ0 deliveries: {}\nIRQ0 currently masked: {}\nSTI currently enabled: {}\noriginal PIC mask restored: {}\nIF disabled before return: {}\nhardware mutation: {}\nruntime irq active: {}\nVGA IRQ0 status: {}\n",
        window.result,
        window.state,
        window.irq0_deliveries,
        window.irq0_currently_masked,
        ...
        window.hardware_mutation,
        window.runtime_irq_active,
        window.vga_irq0_status
    );
}

fn print_irq0_window_clear() {
    use core::fmt::Write;

    let window = irq0_window_clear_snapshot();
    vga_window::draw_irq0_status(window.vga_irq0_status);
    let mut serial_writer = serial::SerialWriter;
    let _ = write!(serial_writer, "IRQ0 delivery one-shot window clear\n{}\nstate: {}\narmed: {}\nIRQ0 deliveries: {}\nhardware mutation: {}\nruntime irq active: {}\n",
        window.result,
        window.state,
        window.armed,
        window.irq0_deliveries,
...
fn print_irq0_ticks_status() {
    use core::fmt::Write;

    let ticks = irq0_ticks_status_snapshot();
    let mut serial_writer = serial::SerialWriter;
    let _ = write!(serial_writer, "IRQ0 tick counter window\nstate: {}\ntarget ticks: {}\nobserved ticks: {}\nIRQ0 currently masked: {}\nSTI currently enabled: {}\noriginal PIC mask restored: {}\nIF disabled before return: {}\nruntime irq active: {}\npreconditions:\n- PIC remap proof: {}\n- manual PIC_EOI proof: {}\n- IRQ0 descriptor bind proof: {}\n- transactional IRQ0 unmask proof: {}\nunmet preconditions: {}\n",
        ticks.state,
        ticks.target_ticks,
        ticks.observed_ticks,
        ticks.irq0_currently_masked,
        ticks.sti_currently_enabled,
        ticks.original_pic_mask_restored,
        ticks.if_disabled_before_return,
        ticks.runtime_irq_active,
        ticks.pic_remap_proof,
        ticks.manual_pic_eoi_proof,
        ticks.irq0_descriptor_bind_proof,
        ticks.transactional_irq0_unmask_proof,
        ticks.unmet_preconditions
    );
}

fn print_irq0_ticks_arm() {
    use core::fmt::Write;

    let ticks = irq0_ticks_arm_snapshot();
    let mut serial_writer = serial::SerialWriter;
    let _ = write!(serial_writer, "IRQ0 tick counter window arm\n{}\nstate: {}\narmed: {}\ntarget ticks: {}\nobserved ticks: {}\nhardware mutation: {}\nruntime irq active: {}\nunmet preconditions: {}\n",
        ticks.result,
        ticks.state,
        ticks.armed,
        ticks.target_ticks,
        ticks.observed_ticks,
        ticks.hardware_mutation,
        ticks.runtime_irq_active,
        ticks.unmet_preconditions
    );
}

fn print_irq0_ticks_fire() {
    use core::fmt::Write;

    let ticks = irq0_ticks_fire_snapshot();
    vga_window::draw_irq0_status(ticks.vga_irq0_status);
    let mut serial_writer = serial::SerialWriter;
    let _ = write!(serial_writer, "IRQ0 tick counter window fire\n{}\nstate: {}\ntarget ticks: {}\nobserved ticks: {}\nIRQ0 currently masked: {}\nSTI currently enabled: {}\noriginal PIC mask restored: {}\nIF disabled before return: {}\nhardware mutation: {}\nruntime irq active: {}\nVGA IRQ0 status: {}\n",
        ticks.result,
        ticks.state,
        ticks.target_ticks,
        ticks.observed_ticks,
        ...
        ticks.hardware_mutation,
        ticks.runtime_irq_active,
        ticks.vga_irq0_status
    );
}

fn print_irq0_ticks_clear() {
    use core::fmt::Write;

    let ticks = irq0_ticks_clear_snapshot();
    vga_window::draw_irq0_status(ticks.vga_irq0_status);
    let mut serial_writer = serial::SerialWriter;
    let _ = write!(serial_writer, "IRQ0 tick counter window clear\n{}\nstate: {}\narmed: {}\ntarget ticks: {}\nobserved ticks: {}\nhardware mutation: {}\nruntime irq active: {}\n",
        ticks.result,
        ticks.state,
        ticks.armed,
        ticks.target_ticks,
...
fn print_irq0_runtime_status() {
    use core::fmt::Write;

    let runtime = irq0_runtime_status_snapshot();
    let mut vga_writer = vga::VgaWriter;
    let mut serial_writer = serial::SerialWriter;
    let _ = write!(vga_writer, "IRQ0 runtime status\nIRQ0 TIMER {} {:04}\nruntime ready: {}\nruntime stopped: {}\nIRQ0 currently masked: {}\nSTI currently enabled: {}\nunmet preconditions: {}\n",
        runtime.state,
        runtime.ticks,
        ...
        runtime.irq0_currently_masked,
        runtime.sti_currently_enabled,
        runtime.unmet_preconditions
    );
    let _ = write!(serial_writer, "IRQ0 runtime status\n{}\nIRQ0 TIMER {} {:04}{}\nruntime ready: {}\nruntime stopped: {}\npreconditions:\n- PIC remap proof: {}\n- manual PIC_EOI proof: {}\n- IRQ0 descriptor bind proof: {}\n- transactional IRQ0 unmask proof: {}\nIRQ0 currently masked: {}\nSTI currently enabled: {}\nsaved original master PIC mask: {:#04x}\nfinal masked-exact master PIC mask: {:#04x}\nnon-IRQ0 PIC mask restored: {}\nIRQ0 forced masked: {}\nunmet preconditions: {}\n",
        runtime.result,
        runtime.state,
        runtime.ticks,
        if runtime.irq0_currently_masked == "yes" { " / MASKED" } else { "" },
        runtime.runtime_ready,
        runtime.runtime_stopped,
        runtime.pic_remap_proof,
        runtime.manual_pic_eoi_proof,
        runtime.irq0_descriptor_bind_proof,
        runtime.transactional_irq0_unmask_proof,
        runtime.irq0_currently_masked,
        runtime.sti_currently_enabled,
        runtime.saved_original_master_mask,
        runtime.final_master_mask,
        runtime.non_irq0_pic_mask_restored,
        runtime.irq0_forced_masked,
        runtime.unmet_preconditions
    );
}

fn print_irq0_runtime_start() {
    use core::fmt::Write;

    let runtime = irq0_runtime_start_snapshot();
    let mut vga_writer = vga::VgaWriter;
    let mut serial_writer = serial::SerialWriter;
    let _ = write!(vga_writer, "IRQ0 runtime start\n{}\nIRQ0 TIMER {} {:04}\nruntime ready: {}\nruntime stopped: {}\nIRQ0 currently masked: {}\nSTI currently enabled: {}\nunmet preconditions: {}\n",
        runtime.result,
        runtime.state,
        runtime.ticks,
        runtime.runtime_ready,
    ...
    let _ = write!(serial_writer, "IRQ0 runtime start\n{}\nIRQ0 TIMER {} {:04}{}\nruntime ready: {}\nruntime stopped: {}\npreconditions:\n- PIC remap proof: {}\n- manual PIC_EOI proof: {}\n- IRQ0 descriptor bind proof: {}\n- transactional IRQ0 unmask proof: {}\nIRQ0 currently masked: {}\nSTI currently enabled: {}\nsaved original master PIC mask: {:#04x}\nfinal masked-exact master PIC mask: {:#04x}\nnon-IRQ0 PIC mask restored: {}\nIRQ0 forced masked: {}\nunmet preconditions: {}\n",
        runtime.result,
        runtime.state,
        runtime.ticks,
        if runtime.irq0_currently_masked == "yes" { " / MASKED" } else { "" },
...
fn print_irq0_runtime_stop() {
    use core::fmt::Write;

    let runtime = irq0_runtime_stop_snapshot();
    let mut vga_writer = vga::VgaWriter;
    let mut serial_writer = serial::SerialWriter;
    let _ = write!(vga_writer, "IRQ0 runtime stop\n{}\nIRQ0 TIMER {} {:04} / MASKED\nruntime stopped: {}\nIRQ0 currently masked: {}\nSTI currently enabled: {}\nnon-IRQ0 PIC mask restored: {}\nIRQ0 forced masked: {}\n",
        runtime.result,
        runtime.state,
        runtime.ticks,
        runtime.runtime_stopped,
    ...
    let _ = write!(serial_writer, "IRQ0 runtime stop\n{}\nIRQ0 TIMER {} {:04} / MASKED\nruntime ready: {}\nruntime stopped: {}\nIRQ0 currently masked: {}\nSTI currently enabled: {}\nsaved original master PIC mask: {:#04x}\nfinal masked-exact master PIC mask: {:#04x}\nnon-IRQ0 PIC mask restored: {}\nIRQ0 forced masked: {}\nunmet preconditions: {}\n",
        runtime.result,
        runtime.state,
        runtime.ticks,
        runtime.runtime_ready,
...
fn print_kernel_clock_status() {
    ...
    let _ = write!(
        vga_writer,
        "KERNEL CLOCK\nsource: {}\nruntime: {}\nticks: {:04}\nirq0 masked: {}\nsti enabled: {}\n",
        clock.source, clock.runtime, clock.ticks, clock.irq0_masked, clock.sti_enabled
    );
    let _ = write!(
        serial_writer,
        "KERNEL CLOCK\nsource: {}\nruntime: {}\nticks: {:04}\nirq0 masked: {}\nsti enabled: {}\n",
                            ...
                            if c == '\x08' {
                                ...
                                if LINE_LEN > 0 {
                                        ...
                                        core::str::from_utf8(&LINE_BUFFER[..LINE_LEN])
                                    {
                                        ...
                                        } else if line_str == "gfx-show" {
                                            vga_gfx::enter_mode_13h();
                                            vga_gfx::draw_first_surface();
                                            vga_prompt_already_rendered = true;
                                            serial::print(
                                                "gfx-show: VGA graphics surface rendered\n",
                                            );
                                        } else if line_str == "gfx-console" {
                                            vga_gfx::enter_mode_13h();
                                        ...
                                        } else if line_str == "gfx-console-input" {
                                            vga_gfx::enter_mode_13h();
                                            gfx_console::draw_graphics_console();
                                            run_gfx_console_input_echo();
                                            vga_prompt_already_rendered = true;
                                        ...
                                        } else if line_str == "irq0-window-status" {
                                            print_irq0_window_status();
                                        } else if line_str == "irq0-window-arm" {
                                            print_irq0_window_arm();
                                        } else if line_str == "irq0-window-fire" {
                                            print_irq0_window_fire();
                                            vga_prompt_already_rendered = true;
                                        } else if line_str == "irq0-window-clear" {
                                            print_irq0_window_clear();
                                            vga_prompt_already_rendered = true;
                                        } else if line_str == "irq0-ticks-status" {
                                            print_irq0_ticks_status();
                                        } else if line_str == "irq0-ticks-arm" {
                                            print_irq0_ticks_arm();
                                        } else if line_str == "irq0-ticks-fire" {
                                            print_irq0_ticks_fire();
                                            vga_prompt_already_rendered = true;
                                        } else if line_str == "irq0-ticks-clear" {
                                            print_irq0_ticks_clear();
                                            vga_prompt_already_rendered = true;
                                        } else if line_str == "irq0-runtime-status" {
                                            print_irq0_runtime_status();
                                        } else if line_str == "irq0-runtime-start" {
                                            print_irq0_runtime_start();
                                        } else if line_str == "irq0-runtime-stop" {
                                            print_irq0_runtime_stop();
                                        } else if line_str == "kernel-clock-status" {
                                            print_kernel_clock_status();
                                        } else if line_str == "dbyte-vm-status" {
                                            dbyte_vm_probe::print_status();
                                        } else if line_str == "dbyte-vm-run-probe" {
                                            dbyte_vm_probe::run_probe();
