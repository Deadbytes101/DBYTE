# DByteOS Kernel Lab Guide (v8.3.0)
...
Version `8.3.0` declares the Kernel Exception Subsystem Foundation. The active exception surface is vector `0` divide-by-zero, vector `3` breakpoint, and vector `14` page fault smoke. Status and recovery are exposed through `exception-status`, `exceptions --verbose`, `fault-status`, `pf-status`, `handlers --active`, and `exception-about`.

Version `8.3.0` adds PIC Remap Dry-Run Telemetry. PIC/IRQ remains planned / disabled, PIC remap code is present / not called, `pic-plan`, `irq-map`, and `pic-status --verbose` expose dry-run status only, IRQ vectors `0x20-0x2f` are planned, and keyboard input stays polling-only through PS/2 ports `0x64` / `0x60`.
