...
| `gfx-console-shell`    | Up to four bounded commands via PS/2 polling | Enters VGA Mode 13h, runs a bounded graphics-only session for `help`, `status`, `clear`, `exit`, or deterministic unknown-command results, and prints dispatch proof over serial. |
