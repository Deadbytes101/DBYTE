...
| `gfx-console-shell`    | Up to four bounded commands via PS/2 polling | Enters VGA Mode 13h, runs a bounded graphics-only session for `help`, `status`, `clear`, `vm`, `apps`, `run hello`, `run math`, `exit`, app-not-found, or deterministic unknown-command results, and prints dispatch proof over serial. |
...
3. Type commands and press Enter to execute them. For example:
   ```txt
    ...
    gfx-console-shell: app dispatched: hello
    gfx-console-shell: app dispatched: math
