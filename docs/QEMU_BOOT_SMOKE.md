# DByteOS QEMU Boot Smoke (v10.54.0)

`v10.54.0` is a DByte IRQ0 Runtime Start Stop Foundation release. It adds text-shell `irq0-runtime-status`, `irq0-runtime-start`, and `irq0-runtime-stop` commands for a controlled manual IRQ0 runtime lifecycle. Runtime start requires the existing prepared IRQ0 proof chain and does not auto-remap PIC, auto-bind IDT, or install handlers; runtime stop uses masked-exact restore with `saved_original_master_mask | 0x01`, preserving non-IRQ0 master PIC mask bits while forcing IRQ0 masked. It adds no graphics-shell command, VM opcode, KCALL service id, app registry entry, scheduler, multitasking, loader, heap allocation, dynamic registry, process bridge, IRQ1 path, slave PIC mutation, or serial path change. QEMU proof artifact: `tmp\qemu_irq0_runtime_start_stop_v10.54.0.serial.log`.
...
Pinned `v10.54.0` IRQ0 runtime serial proof strings are: `irq0-runtime-status`, `IRQ0 TIMER STOPPED`, `irq0-runtime-start`, `irq0 runtime: already running`, `IRQ0 TIMER RUNNING`, `irq0-runtime-stop`, `irq0 runtime: not running`, `runtime stopped: yes`, `non-IRQ0 PIC mask restored: yes`, `IRQ0 forced masked: yes`, and `IRQ0 currently masked: yes`.

Pinned `v10.54.0` QEMU command flow: run `irq0-runtime-status`, run the existing prepared IRQ0 setup proof commands, run `irq0-runtime-status`, run `irq0-runtime-start`, run `irq0-runtime-start` again for the already-running guard, wait briefly, run `irq0-runtime-status`, run `irq0-runtime-stop`, run `irq0-runtime-stop` again for the not-running guard, and run `irq0-runtime-status`.
