# DByteOS QEMU Boot Smoke (v10.53.0)

`v10.53.0` is a DByte App Info Foundation release. It keeps the existing `gfx-console-shell` kernel command, generic `run <app_name>` runner, and bounded graphics-shell session while adding read-only app introspection through `info <app_name>`. The info command looks up the static embedded app registry, renders static app service metadata, and never executes bytecode or mutates `last` session state. It adds no VM opcode, KCALL service id, app registry entry, kernel shell command, loader, heap allocation, dynamic registry, process bridge, or IRQ/PIC/IDT behavior change. QEMU proof artifacts: `tmp\qemu_gfx_console_app_info_v10.53.0.serial.log`, `tmp\qemu_gfx_console_app_info_v10.53.0.ppm`, and `tmp\qemu_gfx_console_app_info_v10.53.0.png`.
...
Pinned `v10.53.0` graphics app-info serial proofs are: `gfx-console-shell: command dispatched: info uidemo`, `gfx-console-shell: command dispatched: info errtest`, `gfx-console-shell: app info not found: nope`, and `gfx-console-shell: exit`.

Pinned `v10.53.0` graphics app-info proof strings are: `command: info uidemo`, `APP uidemo`, `STATUS FOUND`, `SERVICES 7 6 1 2`, `RESULT READY`, `command: info errtest`, `APP errtest`, `SERVICES 99`, `RESULT VM ERROR TEST`, `command: info nope`, `APP nope`, and `STATUS NOT FOUND`.
