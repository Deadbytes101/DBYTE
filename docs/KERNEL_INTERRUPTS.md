# DByteOS Kernel Interrupt Architecture Foundation (v8.11.1)
...
Page Fault handling is **active smoke** in `v8.11.1`. The `pf-smoke` command triggers a controlled real Page Fault through a null read probe, reads `CR2`, decodes the CPU-pushed error code as a raw value, and returns to the shell through a recovery trampoline.
...
Exact bit set tracked for v8.11.1: `P / W/R / U/S / RSVD / I/D`.
...
> The PIC remap code foundation is present, and `v8.11.1` hardens state telemetry for the controlled smoke path. Initialization Command Words (ICWs) are sent only by the explicit two-step `pic-remap-arm` / `pic-remap-smoke` command path; boot, IDT setup, IRQ setup, EOI paths, and keyboard polling do not remap the PIC.
