...
## v9.0.0 IRQ Gate Bind State Telemetry & Static Guards
...
The 9.0.0 milestone introduces the safety latch foundation for runtime IRQ activation.
...
- **IRQ0 timer**: skeleton planned PIT timer interrupt; bind smoke stub is dormant in `v9.0.0`.
- **IRQ1 keyboard**: skeleton planned PS/2 keyboard interrupt; bind smoke stub is dormant in `v9.0.0`.
...
- **STI**: Set Interrupt Flag instruction; not used in `v9.0.0`.
