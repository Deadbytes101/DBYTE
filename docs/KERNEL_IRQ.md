...
`v10.17.0` adds a read-only bridge above the manual PIC_EOI hardware smoke. The bridge reports whether the current session has proven the manual smoke, then keeps handler-triggered EOI and runtime IRQ activation denied.
