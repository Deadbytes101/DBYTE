# DByteOS Kernel Exception Subsystem Foundation (v9.0.0)

DByteOS Kernel Lab `v9.0.0` preserves the exception subsystem foundation while adding PIC/IRQ direction docs separately in `KERNEL_IRQ.md`. This release does not add new exception vectors, change Page Fault smoke mechanics, enable STI, remap PIC, bind IRQ vectors, or replace keyboard polling.
