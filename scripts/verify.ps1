...
if ($versionOut -ne "DByte 8.5.0") {
    throw "Version mismatch: expected 'DByte 8.5.0', got '$versionOut'"
}
...
Assert-Contains $shellBasic.Text "DByte 8.5.0" "shell version"
...
Write-Host "Running DByteOS Command Set (v8.5.0) smoke tests..."
...
==================================================
System:
  Version:    DByte  8.5.0  ( Userland Prototype )
...
try {
    ...
    # --- v8.5.0 Exact Snapshot Assertions ---
...
"@
    ...
    Assert-Equal $dbyteosPromptDefault.Text "dbyte-shell> DByte 8.5.0`ndbyte-shell>" "dbyteos shell prompt default snapshot"
    ...
    Assert-Equal $dbyteosPromptChange.Text "dbyte-shell> preference 'system.prompt' updated successfully.`ndbyteos> DByte 8.5.0`ndbyteos> preference 'system.prompt' updated successfully.`ndeadbyte> DByte 8.5.0`ndeadbyte> preferences reset to default seed state.`ndbyte-shell> DByte 8.5.0`ndbyte-shell>" "dbyteos shell prompt preference snapshots"
    ...
    Assert-Contains $dbyteosSysinfoRoot.Text "version: DByte 8.5.0" "dbyteos sysinfo version"
    ...
    Assert-Contains $dbyteosProfileRoot.Text "os_version: 8.5.0" "dbyteos profile os version"
    ...
    Write-Host "Running DByteOS Security/Permissions (v8.5.0) smoke tests..."
    ...
    Assert-Contains $dbyteosReadEtc.Text "pub let os_version: str = `"8.5.0`"" "read etc allowed"
    ...
    Write-Host "Running DByteOS Security Enforcement Expansion (v8.5.0) smoke tests..."
    $enforcementInput = @"
clean
cat etc/system.dby
...
"@
    ...
    Assert-Contains $dbyteosEnforcement.Text "os_version: str = `"8.5.0`"" "cat etc allowed"
    ...
    Write-Host "Running DByteOS Security Enforcement Hardening (v8.5.0) smoke tests..."
    $hardeningInput = @"
clean
cat boot.dby
touch boot.dby
...
"@
    ...
    Write-Host "Verifying DByteOS Personal Workspace Beta Foundation (v8.5.0) documentation..."
    ...
    Assert-Contains $mainReadme "DByteOS Personal Workspace Beta Foundation (v8.5.0)" "README Personal Workspace Beta Foundation positioning"
    ...
    Assert-Contains $osReadme "DByteOS Personal Workspace Beta Foundation (v8.5.0)" "OS README Personal Workspace Beta Foundation positioning"
    ...
    Assert-Contains $configGuide "read-only in v8.5.0" "config guide read-only"
    ...
    Assert-Contains $snapshotGuide "read-only in v8.5.0" "snapshot guide read-only"
    ...
    Assert-Contains $projectsGuide "v8.5.0 hardening" "projects guide hardening"
    ...
    # v8.5.0 enforcement confirmed via smoke tests above
    ...
    Assert-Contains $dbyteosCmdShell.Text "version: DByte 8.5.0" "dbyteos shell sysinfo"
    ...
    Assert-Contains $dbyteosCmdShell.Text "os_version: 8.5.0" "dbyteos shell profile"
    ...
    Write-Host "Running DByteOS Notes Workflow (v8.5.0) smoke tests..."
    ...
    Write-Host "Running DByteOS Notes Hardening (v8.5.0) smoke tests..."
    $notesInput = @"
clean
notes read
notes add
...
"@
    ...
    Write-Host "Running DByteOS Init Services (v8.5.0) smoke tests..."
    ...
    Write-Host "Running DByteOS Journal/Logger (v8.5.0) smoke tests..."
...
"@
    ...
    Write-Host "Running DByteOS Search (v8.5.0) smoke tests..."
    ...
    # --- v8.5.0 Deterministic Rejections ---
    ...
    # --- v8.5.0 Search UX Missing Cache / Scan Fallback tests ---
    ...
    # --- v8.5.0 Search UX Active Cache tests ---
...
"@
    ...
    # --- DByteOS Workspace Dashboard (v8.5.0) tests ---
...
Write-Host "Verifying DByteOS Kernel Lab (v8.5.0) freestanding build..."
...
Write-Host "Verifying DByteOS Kernel Lab (v8.5.0) exception status UX contracts..."
...
Assert-Contains $kernelPicSource "v8.5.0 keeps the dry-run remap plan disabled while IRQ0/IRQ1 handler" "pic source skeleton milestone wording"
...
Assert-Contains $kernelInterruptDocs 'Exact bit set tracked for v8.5.0: `P / W/R / U/S / RSVD / I/D`.' "interrupt docs exact error-code bit names"
...
Assert-Contains $kernelIrqDocs '**IRQ0 timer**: skeleton planned PIT timer interrupt; disabled in `v8.5.0`.' "irq docs irq0 skeleton disabled"
Assert-Contains $kernelIrqDocs '**IRQ1 keyboard**: skeleton planned PS/2 keyboard interrupt; disabled in `v8.5.0`.' "irq docs irq1 skeleton disabled"
...
Assert-Contains $kernelIrqDocs "EOI strategy: planned / disabled" "irq docs pic eoi-status snapshot"
Assert-Contains $kernelIrqDocs "EOI strategy note:" "irq docs eoi-note snapshot"
...
Assert-Contains $kernelBootSmokeDocs "commands: help about version clear echo mem uptime banner keyboard reboot-note system cls status mods keys prompt int3 div0 exception exception-reset handlers handlers --active exception-status exceptions exceptions --verbose exception-help exception-about fault-status fault-reset pf-note pf-status pf-smoke irq-note irq-status irq-handlers eoi-note eoi-status pic-note pic-status pic-plan irq-map pic-status --verbose" "qemu docs help snapshot"
...
Assert-Contains $kernelBootSmokeDocs "eoi strategy status:" "qemu docs eoi strategy status output"
...
Assert-Contains $kernelBootSmokeDocs "End-Of-Interrupt" "qemu docs eoi explanation"
...
# ==============================================================================
# DByteOS Kernel Lab Hardening Verify Guards (v8.5.0)
# ==============================================================================
Write-Host "Verifying DByteOS Kernel Lab (v8.5.0) hardening guards..." -ForegroundColor Green
...
if (-not $qemuExe) {
    ...
    Write-Host "Verifying DByteOS Kernel Lab (v8.5.0) virtualized QEMU boot smoke using $($qemuExe.Name)..." -ForegroundColor Green
    ...
    if (-not ($logContent -like "*version: 8.5.0*")) {
        throw "QEMU boot smoke test failed: 'version: 8.5.0' not found in serial log!"
    }
...
Write-Host "Running DByteOS Alpha (v8.5.0) Package Smoke Tests..."
...
& .\scripts\package_release.ps1 -Version "8.5.0"
$zipFile = Join-Path $repoRoot "dbyte-v8.5.0-windows-x64.zip"
...
if ($vOut -ne "DByte 8.5.0") { throw "Package version mismatch: $vOut" }
...
try {
    ...
    Assert-Contains (Normalize-Output $pkgDashboardSnapshot) "OS Version: 8.5.0" "Package dashboard snapshot"
    ...
    Assert-Contains (Normalize-Output $pkgJourneySnapshot) "DByte 8.5.0" "Package Beta Journey Snapshot"
...
Assert-Contains (Normalize-Output $shellOut) "OS:      DByte  8.5.0" "Package shell status version"
Assert-Contains (Normalize-Output $shellOut) "version: DByte 8.5.0" "Package shell sysinfo version"
...
Assert-Equal $packagePromptDefault.Text "dbyte-shell> DByte 8.5.0`ndbyte-shell>" "Package shell prompt default snapshot"
...
Assert-Equal $packagePromptChange.Text "dbyte-shell> preference 'system.prompt' updated successfully.`ndbyteos> DByte 8.5.0`ndbyteos> preference 'system.prompt' updated successfully.`ndeadbyte> DByte 8.5.0`ndeadbyte> preferences reset to default seed state.`ndbyte-shell> DByte 8.5.0`ndbyte-shell>" "Package shell prompt preference snapshots"
