...
if ($versionOut -ne "DByte 4.6.1") {
    throw "Version mismatch: expected 'DByte 4.6.1', got '$versionOut'"
}
...
Assert-Contains $shellBasic.Text "DByte 4.6.1" "shell version"
...
Write-Host "Running DByteOS Command Set (v4.6.1) smoke tests..."
...
==================================================
System:
  Version:    DByte  4.6.1  ( Userland Prototype )
...
usage: diagnose [profile|config|security|logs|manual|package]
"@
...
try {
    ...
    Assert-Contains $dbyteosSysinfoRoot.Text "version: DByte 4.6.1" "dbyteos sysinfo version"
    ...
    Assert-Contains $dbyteosProfileRoot.Text "os_version: 4.6.1" "dbyteos profile os version"
    ...
    Write-Host "Running DByteOS Security/Permissions (v4.6.1) smoke tests..."
    ...
    Assert-Contains $dbyteosReadEtc.Text "pub let os_version: str = `"4.6.1`"" "read etc allowed"
    ...
    Write-Host "Running DByteOS Security Enforcement Expansion (v4.6.1) smoke tests..."
    $enforcementInput = @"
clean
cat etc/system.dby
...
"@
    ...
    Assert-Contains $dbyteosEnforcement.Text "os_version: str = `"4.6.1`"" "cat etc allowed"
    ...
    Write-Host "Running DByteOS Security Enforcement Hardening (v4.6.1) smoke tests..."
    $hardeningInput = @"
clean
cat boot.dby
touch boot.dby
...
"@
    ...
    Write-Host "Verifying DByteOS Alpha Userland (v4.6.1) documentation..."
    ...
    Assert-Contains $mainReadme "DByteOS Alpha Userland (v4.6.1)" "README alpha positioning"
    ...
    Assert-Contains $osReadme "DByteOS Alpha Userland (v4.6.1)" "OS README alpha positioning"
    ...
    Assert-Contains $configGuide "read-only in v4.6.1" "config guide read-only"
    ...
    Assert-Contains $snapshotGuide "read-only in v4.6.1" "snapshot guide read-only"
    ...
    # v4.6.1 enforcement confirmed via smoke tests above
    ...
    Assert-Contains $dbyteosCmdShell.Text "version: DByte 4.6.1" "dbyteos shell sysinfo"
    ...
    Assert-Contains $dbyteosCmdShell.Text "os_version: 4.6.1" "dbyteos shell profile"
    ...
    Write-Host "Running DByteOS Notes Workflow (v4.6.1) smoke tests..."
    ...
    Write-Host "Running DByteOS Notes Hardening (v4.6.1) smoke tests..."
    $notesInput = @"
clean
notes read
notes add
...
"@
    ...
    Write-Host "Running DByteOS Init Services (v4.6.1) smoke tests..."
    ...
    Write-Host "Running DByteOS Journal/Logger (v4.6.1) smoke tests..."
...
"@
    ...
    Assert-Equal $dbyteosDiagnoseUnknown.Text "usage: diagnose [profile|config|security|logs|manual|package]" "dbyteos diagnose unknown snapshot"
...
Write-Host "Running DByteOS Alpha (v4.6.1) Package Smoke Tests..."
...
& powershell -ExecutionPolicy Bypass -File .\scripts\package_release.ps1 -Version "4.6.1"
$zipFile = Join-Path $repoRoot "dbyte-v4.6.1-windows-x64.zip"
...
if ($vOut -ne "DByte 4.6.1") { throw "Package version mismatch: $vOut" }
...
Assert-Equal $diagnoseUnknownOut "usage: diagnose [profile|config|security|logs|manual|package]" "Package diagnose unknown snapshot"
...
$expectedDocs = @("DBYTEOS_ALPHA.md", "DBYTEOS_COMMANDS.md", "DBYTEOS_SECURITY.md", "DBYTEOS_BOOT.md", "DBYTEOS_PACKAGE.md", "DBYTEOS_ONBOARDING.md", "DBYTEOS_PROFILE.md", "DBYTEOS_CONFIG.md", "DBYTEOS_SNAPSHOT.md", "DBYTEOS_DIAGNOSTICS.md")
...
Assert-Contains (Normalize-Output $shellOut) "OS:      DByte  4.6.1" "Package shell status version"
Assert-Contains (Normalize-Output $shellOut) "version: DByte 4.6.1" "Package shell sysinfo version"
