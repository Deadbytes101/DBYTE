...
if ($versionOut -ne "DByte 6.5.1") {
    throw "Version mismatch: expected 'DByte 6.5.1', got '$versionOut'"
}
...
Assert-Contains $shellBasic.Text "DByte 6.5.1" "shell version"
...
Write-Host "Running DByteOS Command Set (v6.5.1) smoke tests..."
...
==================================================
System:
  Version:    DByte  6.5.1  ( Userland Prototype )
...
try {
    ...
    # --- v6.5.1 Exact Snapshot Assertions ---
...
"@
    ...
    Assert-Equal $dbyteosPromptDefault.Text "dbyte-shell> DByte 6.5.1`ndbyte-shell>" "dbyteos shell prompt default snapshot"
    ...
    Assert-Equal $dbyteosPromptChange.Text "dbyte-shell> preference 'system.prompt' updated successfully.`ndbyteos> DByte 6.5.1`ndbyteos> preference 'system.prompt' updated successfully.`ndeadbyte> DByte 6.5.1`ndeadbyte> preferences reset to default seed state.`ndbyte-shell> DByte 6.5.1`ndbyte-shell>" "dbyteos shell prompt preference snapshots"
    ...
    Assert-Contains $dbyteosSysinfoRoot.Text "version: DByte 6.5.1" "dbyteos sysinfo version"
    ...
    Assert-Contains $dbyteosProfileRoot.Text "os_version: 6.5.1" "dbyteos profile os version"
    ...
    Write-Host "Running DByteOS Security/Permissions (v6.5.1) smoke tests..."
    ...
    Assert-Contains $dbyteosReadEtc.Text "pub let os_version: str = `"6.5.1`"" "read etc allowed"
    ...
    Write-Host "Running DByteOS Security Enforcement Expansion (v6.5.1) smoke tests..."
    $enforcementInput = @"
clean
cat etc/system.dby
...
"@
    ...
    Assert-Contains $dbyteosEnforcement.Text "os_version: str = `"6.5.1`"" "cat etc allowed"
    ...
    Write-Host "Running DByteOS Security Enforcement Hardening (v6.5.1) smoke tests..."
    $hardeningInput = @"
clean
cat boot.dby
touch boot.dby
...
"@
    ...
    Write-Host "Verifying DByteOS Personal Workspace Beta Foundation (v6.5.1) documentation..."
    ...
    Assert-Contains $mainReadme "DByteOS Personal Workspace Beta Foundation (v6.5.1)" "README Personal Workspace Beta Foundation positioning"
    ...
    Assert-Contains $osReadme "DByteOS Personal Workspace Beta Foundation (v6.5.1)" "OS README Personal Workspace Beta Foundation positioning"
    ...
    Assert-Contains $configGuide "read-only in v6.5.1" "config guide read-only"
    ...
    Assert-Contains $snapshotGuide "read-only in v6.5.1" "snapshot guide read-only"
    ...
    Assert-Contains $projectsGuide "v6.5.1 hardening" "projects guide hardening"
    ...
    # v6.5.1 enforcement confirmed via smoke tests above
    ...
    Assert-Contains $dbyteosCmdShell.Text "version: DByte 6.5.1" "dbyteos shell sysinfo"
    ...
    Assert-Contains $dbyteosCmdShell.Text "os_version: 6.5.1" "dbyteos shell profile"
    ...
    Write-Host "Running DByteOS Notes Workflow (v6.5.1) smoke tests..."
    ...
    Write-Host "Running DByteOS Notes Hardening (v6.5.1) smoke tests..."
    $notesInput = @"
clean
notes read
notes add
...
"@
    ...
    Write-Host "Running DByteOS Init Services (v6.5.1) smoke tests..."
    ...
    Write-Host "Running DByteOS Journal/Logger (v6.5.1) smoke tests..."
...
"@
    ...
    Write-Host "Running DByteOS Search (v6.5.1) smoke tests..."
    ...
    # --- v6.5.1 Deterministic Rejections ---
    ...
    # --- v6.5.1 Search UX Missing Cache / Scan Fallback tests ---
    ...
    # --- v6.5.1 Search UX Active Cache tests ---
...
"@
    ...
    # --- DByteOS Workspace Dashboard (v6.5.1) tests ---
...
Write-Host "Verifying DByteOS Kernel Lab (v6.5.1) freestanding build..."
...
if (-not $qemuExe) {
    ...
    Write-Host "Verifying DByteOS Kernel Lab (v6.5.1) virtualized QEMU boot smoke using $($qemuExe.Name)..." -ForegroundColor Green
    ...
    if (-not ($logContent -like "*version: 6.5.1*")) {
        throw "QEMU boot smoke test failed: 'version: 6.5.1' not found in serial log!"
    }
...
Write-Host "Running DByteOS Alpha (v6.5.1) Package Smoke Tests..."
...
& .\scripts\package_release.ps1 -Version "6.5.1"
$zipFile = Join-Path $repoRoot "dbyte-v6.5.1-windows-x64.zip"
...
if ($vOut -ne "DByte 6.5.1") { throw "Package version mismatch: $vOut" }
...
try {
    ...
    Assert-Contains (Normalize-Output $pkgDashboardSnapshot) "OS Version: 6.5.1" "Package dashboard snapshot"
    ...
    Assert-Contains (Normalize-Output $pkgJourneySnapshot) "DByte 6.5.1" "Package Beta Journey Snapshot"
...
Assert-Contains (Normalize-Output $shellOut) "OS:      DByte  6.5.1" "Package shell status version"
Assert-Contains (Normalize-Output $shellOut) "version: DByte 6.5.1" "Package shell sysinfo version"
...
Assert-Equal $packagePromptDefault.Text "dbyte-shell> DByte 6.5.1`ndbyte-shell>" "Package shell prompt default snapshot"
...
Assert-Equal $packagePromptChange.Text "dbyte-shell> preference 'system.prompt' updated successfully.`ndbyteos> DByte 6.5.1`ndbyteos> preference 'system.prompt' updated successfully.`ndeadbyte> DByte 6.5.1`ndeadbyte> preferences reset to default seed state.`ndbyte-shell> DByte 6.5.1`ndbyte-shell>" "Package shell prompt preference snapshots"
