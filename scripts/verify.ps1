...
if ($versionOut -ne "DByte 5.4.0") {
    throw "Version mismatch: expected 'DByte 5.4.0', got '$versionOut'"
}
...
Assert-Contains $shellBasic.Text "DByte 5.4.0" "shell version"
...
Write-Host "Running DByteOS Command Set (v5.4.0) smoke tests..."
...
==================================================
System:
  Version:    DByte  5.4.0  ( Userland Prototype )
...
try {
    ...
    # --- v5.4.0 Exact Snapshot Assertions ---
...
"@
    ...
    Assert-Equal $dbyteosPromptDefault.Text "dbyte-shell> DByte 5.4.0`ndbyte-shell>" "dbyteos shell prompt default snapshot"
    ...
    Assert-Equal $dbyteosPromptChange.Text "dbyte-shell> preference 'system.prompt' updated successfully.`ndbyteos> DByte 5.4.0`ndbyteos> preference 'system.prompt' updated successfully.`ndeadbyte> DByte 5.4.0`ndeadbyte> preferences reset to default seed state.`ndbyte-shell> DByte 5.4.0`ndbyte-shell>" "dbyteos shell prompt preference snapshots"
    ...
    Assert-Contains $dbyteosSysinfoRoot.Text "version: DByte 5.4.0" "dbyteos sysinfo version"
    ...
    Assert-Contains $dbyteosProfileRoot.Text "os_version: 5.4.0" "dbyteos profile os version"
    ...
    Write-Host "Running DByteOS Security/Permissions (v5.4.0) smoke tests..."
    ...
    Assert-Contains $dbyteosReadEtc.Text "pub let os_version: str = `"5.4.0`"" "read etc allowed"
    ...
    Write-Host "Running DByteOS Security Enforcement Expansion (v5.4.0) smoke tests..."
    $enforcementInput = @"
clean
cat etc/system.dby
...
"@
    ...
    Assert-Contains $dbyteosEnforcement.Text "os_version: str = `"5.4.0`"" "cat etc allowed"
    ...
    Write-Host "Running DByteOS Security Enforcement Hardening (v5.4.0) smoke tests..."
    $hardeningInput = @"
clean
cat boot.dby
touch boot.dby
...
"@
    ...
    Write-Host "Verifying DByteOS Personal Alpha (v5.4.0) documentation..."
    ...
    Assert-Contains $mainReadme "DByteOS Personal Alpha (v5.4.0)" "README personal alpha positioning"
    ...
    Assert-Contains $osReadme "DByteOS Personal Alpha (v5.4.0)" "OS README personal alpha positioning"
    ...
    Assert-Contains $configGuide "read-only in v5.4.0" "config guide read-only"
    ...
    Assert-Contains $snapshotGuide "read-only in v5.4.0" "snapshot guide read-only"
    ...
    Assert-Contains $projectsGuide "v5.4.0 hardening" "projects guide hardening"
    ...
    # v5.4.0 enforcement confirmed via smoke tests above
    ...
    Assert-Contains $dbyteosCmdShell.Text "version: DByte 5.4.0" "dbyteos shell sysinfo"
    ...
    Assert-Contains $dbyteosCmdShell.Text "os_version: 5.4.0" "dbyteos shell profile"
    ...
    Write-Host "Running DByteOS Notes Workflow (v5.4.0) smoke tests..."
    ...
    Write-Host "Running DByteOS Notes Hardening (v5.4.0) smoke tests..."
    $notesInput = @"
clean
notes read
notes add
...
"@
    ...
    Write-Host "Running DByteOS Init Services (v5.4.0) smoke tests..."
    ...
    Write-Host "Running DByteOS Journal/Logger (v5.4.0) smoke tests..."
...
Write-Host "Running DByteOS Alpha (v5.4.0) Package Smoke Tests..."
...
& powershell -ExecutionPolicy Bypass -File .\scripts\package_release.ps1 -Version "5.4.0"
$zipFile = Join-Path $repoRoot "dbyte-v5.4.0-windows-x64.zip"
...
if ($vOut -ne "DByte 5.4.0") { throw "Package version mismatch: $vOut" }
...
Assert-Contains (Normalize-Output $shellOut) "OS:      DByte  5.4.0" "Package shell status version"
Assert-Contains (Normalize-Output $shellOut) "version: DByte 5.4.0" "Package shell sysinfo version"
...
Assert-Equal $packagePromptDefault.Text "dbyte-shell> DByte 5.4.0`ndbyte-shell>" "Package shell prompt default snapshot"
...
Assert-Equal $packagePromptChange.Text "dbyte-shell> preference 'system.prompt' updated successfully.`ndbyteos> DByte 5.4.0`ndbyteos> preference 'system.prompt' updated successfully.`ndeadbyte> DByte 5.4.0`ndeadbyte> preferences reset to default seed state.`ndbyte-shell> DByte 5.4.0`ndbyte-shell>" "Package shell prompt preference snapshots"
...
$packageJourneyInput = "boot`nwelcome`ncheck-system`ndoctor`nprefs set system.prompt dbyteos>`nsnapshot`nproject reset-demo`ntask reset-demo`ntask list demo`ntask add demo write tests`ntask done demo 1`ntask status demo`ntask summary demo`ntask open demo`ntask doctor demo`ntask snapshot demo`ntask clear-done demo`nproject status demo`nproject snapshot demo`nprefs reset-demo`nversion`nquit`n"
