...
if ($versionOut -ne "DByte 8.9.1") {
    throw "Version mismatch: expected 'DByte 8.9.1', got '$versionOut'"
}
...
Assert-Contains $shellBasic.Text "DByte 8.9.1" "shell version"
...
Write-Host "Running DByteOS Command Set (v8.9.1) smoke tests..."
...
==================================================
System:
  Version:    DByte  8.9.1  ( Userland Prototype )
...
try {
    ...
    # --- v8.9.1 Exact Snapshot Assertions ---
...
"@
    ...
    Assert-Equal $dbyteosPromptDefault.Text "dbyte-shell> DByte 8.9.1`ndbyte-shell>" "dbyteos shell prompt default snapshot"
    ...
    Assert-Equal $dbyteosPromptChange.Text "dbyte-shell> preference 'system.prompt' updated successfully.`ndbyteos> DByte 8.9.1`ndbyteos> preference 'system.prompt' updated successfully.`ndeadbyte> DByte 8.9.1`ndeadbyte> preferences reset to default seed state.`ndbyte-shell> DByte 8.9.1`ndbyte-shell>" "dbyteos shell prompt preference snapshots"
    ...
    Assert-Contains $dbyteosSysinfoRoot.Text "version: DByte 8.9.1" "dbyteos sysinfo version"
    ...
    Assert-Contains $dbyteosProfileRoot.Text "os_version: 8.9.1" "dbyteos profile os version"
    ...
    Write-Host "Running DByteOS Security/Permissions (v8.9.1) smoke tests..."
    ...
    Assert-Contains $dbyteosReadEtc.Text "pub let os_version: str = `"8.9.1`"" "read etc allowed"
    ...
    Write-Host "Running DByteOS Security Enforcement Expansion (v8.9.1) smoke tests..."
    $enforcementInput = @"
clean
cat etc/system.dby
...
"@
    ...
    Assert-Contains $dbyteosEnforcement.Text "os_version: str = `"8.9.1`"" "cat etc allowed"
    ...
    Write-Host "Running DByteOS Security Enforcement Hardening (v8.9.1) smoke tests..."
    $hardeningInput = @"
clean
cat boot.dby
touch boot.dby
...
"@
    ...
    Write-Host "Verifying DByteOS Personal Workspace Beta Foundation (v8.9.1) documentation..."
    ...
    Assert-Contains $mainReadme "DByteOS Personal Workspace Beta Foundation (v8.9.1)" "README Personal Workspace Beta Foundation positioning"
    ...
    Assert-Contains $osReadme "DByteOS Personal Workspace Beta Foundation (v8.9.1)" "OS README Personal Workspace Beta Foundation positioning"
    ...
    Assert-Contains $configGuide "read-only in v8.9.1" "config guide read-only"
    ...
    Assert-Contains $snapshotGuide "read-only in v8.9.1" "snapshot guide read-only"
    ...
    Assert-Contains $projectsGuide "v8.9.1 disabled path foundation" "projects guide hardening"
    ...
    # v8.9.1 enforcement confirmed via smoke tests above
    ...
    Assert-Contains $dbyteosCmdShell.Text "version: DByte 8.9.1" "dbyteos shell sysinfo"
    ...
    Assert-Contains $dbyteosCmdShell.Text "os_version: 8.9.1" "dbyteos shell profile"
    ...
    Write-Host "Running DByteOS Notes Workflow (v8.9.1) smoke tests..."
    ...
    Write-Host "Running DByteOS Notes Hardening (v8.9.1) smoke tests..."
    $notesInput = @"
clean
notes read
notes add
...
"@
    ...
    Write-Host "Running DByteOS Init Services (v8.9.1) smoke tests..."
    ...
    Write-Host "Running DByteOS Journal/Logger (v8.9.1) smoke tests..."
...
"@
    ...
    Write-Host "Running DByteOS Search (v8.9.1) smoke tests..."
    ...
    # --- v8.9.1 Deterministic Rejections ---
    ...
    # --- v8.9.1 Search UX Missing Cache / Scan Fallback tests ---
    ...
    # --- v8.9.1 Search UX Active Cache tests ---
    ...
    $uxProjectsEmpty = Invoke-Dbyte -Arguments @("run", "bin\search.dby", "projects", '""') -WorkingDirectory $dbyteosRoot
    ...
    $cacheIndexSearchEmpty = Invoke-Dbyte -Arguments @("run", "bin\search.dby", "index", '""') -WorkingDirectory $dbyteosRoot
...
"@
    ...
    # --- DByteOS Workspace Dashboard (v8.9.1) tests ---
...
Write-Host "Verifying DByteOS Kernel Lab (v8.9.1) freestanding build..."
...
Write-Host "Verifying DByteOS Kernel Lab (v8.9.1) exception status UX contracts..."
...
Assert-Contains $kernelMainSource "exceptions handled: {}`r`nlast exception: none`r`n" "kernel system none telemetry output"
Assert-Contains $kernelMainSource "exceptions handled: {}`r`nlast exception: {} ({})`r`n" "kernel system populated telemetry output"
...
$expectedIrqReadinessOutput = "IRQ runtime readiness`nidt exceptions: ok`nirq gate plan: ok`neoi strategy: ok`npic remap: disabled`nsti: disabled`nkeyboard fallback: polling`nready for runtime irq: no"
Assert-Contains $kernelIrqDocs $expectedIrqReadinessOutput "irq docs irq-readiness exact rendered contract"
$expectedQemuIrqReadinessOutput = "IRQ runtime readiness`n    idt exceptions: ok`n    irq gate plan: ok`n    eoi strategy: ok`n    pic remap: disabled`n    sti: disabled`n    keyboard fallback: polling`n    ready for runtime irq: no"
...
$expectedIrqPreflightOutput = "IRQ runtime preflight`nIDT exceptions 0/3/14: pass`nIRQ vectors 32/33: unbound`nbind path: disabled`nEOI dispatch: disabled`nPIC remap: disabled`nkeyboard fallback: polling`npf-smoke: unchanged`nresult: blocked"
Assert-Contains $kernelIrqDocs $expectedIrqPreflightOutput "irq docs irq-preflight exact rendered contract"
$expectedQemuIrqPreflightOutput = "IRQ runtime preflight`n    IDT exceptions 0/3/14: pass`n    IRQ vectors 32/33: unbound`n    bind path: disabled`n    EOI dispatch: disabled`n    PIC remap: disabled`n    keyboard fallback: polling`n    pf-smoke: unchanged`n    result: blocked"
...
$expectedIdtBindings = @(0, 3, 14)
if (($idtBindings -join ',') -ne ($expectedIdtBindings -join ',')) {
    throw "Kernel IDT vector guard failed: expected bindings 0,3,14 only; found $($idtBindings -join ',')"
}
...
Assert-NotContains ($kernelMainSource.Substring(0, $kernelMainSource.IndexOf('loop {'))) "irq::bind_irq_gates_disabled()" "kernel boot path does not call disabled bind helper"
...
Assert-NotContains $kernelMainSource "IDT.entries[33].set_handler" "kernel does not bind IRQ1 vector 33"
...
Assert-Contains $kernelPicSource "v8.9.1 keeps the dry-run remap plan disabled while IRQ0/IRQ1 handler" "pic source skeleton milestone wording"
...
if ($irq0VectorUses -ne 4 -or $irq1VectorUses -ne 4) {
    throw "IRQ gate vector sync guard failed: expected IRQ0_VECTOR and IRQ1_VECTOR to be used by skeleton, gate plan, disabled bind status, and disabled bind step exactly four times; found IRQ0=$irq0VectorUses IRQ1=$irq1VectorUses"
}
...
Assert-Contains $kernelInterruptDocs "EOI Strategy Foundation" "interrupt docs mark eoi strategy foundation release"
...
Assert-Contains $kernelInterruptDocs "no hardware writes are performed from the PIC module" "interrupt docs no pic hardware writes"
Assert-Contains $kernelInterruptDocs "no IRQ vector binding" "interrupt docs no irq vector binding"
...
Assert-Contains $kernelInterruptDocs "no IRQ1 keyboard active handler" "interrupt docs no irq1 active handler"
Assert-Contains $kernelInterruptDocs "no IRQ0 PIT active handler" "interrupt docs no irq0 active handler"
...
Assert-Contains $kernelInterruptDocs "active/smoke recovery status" "interrupt docs system recovery status"
...
Assert-Contains $kernelInterruptDocs 'Exact bit set tracked for v8.9.1: `P / W/R / U/S / RSVD / I/D`.' "interrupt docs exact error-code bit names"
...
Assert-Contains $kernelIrqDocs "implements an EOI strategy foundation on top of the IRQ handler skeleton" "irq docs skeleton release"
Assert-Contains $kernelIrqDocs "planning and code-foundation-only release" "irq docs skeleton-only release"
Assert-Contains $kernelIrqDocs "remap function is present / not called" "irq docs remap present not called"
Assert-Contains $kernelIrqDocs "PIC/IRQ remains planned / disabled" "irq docs planned disabled"
...
Assert-Contains $kernelIrqDocs "PIC remap dry-run telemetry is documented and compiled only" "irq docs documented compiled only"
Assert-Contains $kernelIrqDocs "No Initialization Command Words are dispatched" "irq docs no icw dispatch"
Assert-Contains $kernelIrqDocs "no hardware writes are performed" "irq docs no hardware writes"
...
Assert-Contains $kernelIrqDocs '**IRQ0 timer**: skeleton planned PIT timer interrupt; disabled in `v8.9.1`.' "irq docs irq0 skeleton disabled"
Assert-Contains $kernelIrqDocs '**IRQ1 keyboard**: skeleton planned PS/2 keyboard interrupt; disabled in `v8.9.1`.' "irq docs irq1 skeleton disabled"
...
Assert-Contains $kernelIrqDocs "No PIC remap call or ICW dispatch." "irq docs no remap call"
Assert-Contains $kernelIrqDocs 'No hardware writes from `kernel-lab/src/pic.rs`.' "irq docs no pic hardware writes boundary"
Assert-Contains $kernelIrqDocs 'No active IRQ IDT bindings beyond existing exception vectors `0`, `3`, and `14`.' "irq docs no irq idt bindings"
Assert-Contains $kernelIrqDocs "No IRQ1 keyboard active handler or IDT binding." "irq docs no irq1 active binding"
Assert-Contains $kernelIrqDocs "No IRQ0 PIT active handler or IDT binding." "irq docs no irq0 active binding"
...
Assert-Contains $kernelIrqDocs "Both gates remain strictly unbound at runtime." "irq docs gates unbound constraint"
...
Assert-Contains $kernelBootSmokeDocs "commands: help about version clear echo mem uptime banner keyboard reboot-note system cls status mods keys prompt int3 div0 exception exception-reset handlers handlers --active exception-status exceptions exceptions --verbose exception-help exception-about fault-status fault-reset pf-note pf-status pf-smoke irq-note irq-status irq-handlers eoi-note eoi-status irq-gates irq-gate-status irq-gate-plan irq-bind-note irq-bind-status irq-readiness irq-risk irq-preflight pic-note pic-status pic-plan irq-map pic-status --verbose" "qemu docs help snapshot"
...
# ==============================================================================
# DByteOS Kernel Lab Hardening Verify Guards (v8.9.1)
# ==============================================================================
Write-Host "Verifying DByteOS Kernel Lab (v8.9.1) hardening guards..." -ForegroundColor Green
...
# 3. Verify guard that PIC/IRQ is still untouched
# Direct writes to Master PIC ports (0x20/0x21) and Slave PIC ports (0xA0/0xA1) must not exist in general source code (except stubs/definitions in pic.rs and serial.rs)
Get-ChildItem (Join-Path $repoRoot "kernel-lab\src") -Filter "*.rs" | ForEach-Object {
...
Write-Host "[OK] Guard passed: PIC and external IRQ controllers remain completely untouched." -ForegroundColor Green
...
# 5.5 Verify guard that IDT entries 32 and 33 are strictly unbound/dormant
Assert-NotContains $kernelMainSource "entries[32].set_handler" "kernel IDT entry 32 remains unbound"
...
Get-ChildItem (Join-Path $repoRoot "kernel-lab\src") -Filter "*.rs" | ForEach-Object {
    if ($_.Name -eq 'pic.rs') {
        $content = Get-Content $_.FullName -Raw
        if ($content -match 'outb\s*\(') {
            throw "Hardening guard failed: Active EOI port write ('outb') is strictly forbidden in pic.rs!"
        }
...
Write-Host "[OK] Guard passed: EOI dispatch is dry-run only (no 'outb' writes in pic.rs)." -ForegroundColor Green
...
if (-not $qemuExe) {
    ...
    Write-Host "Verifying DByteOS Kernel Lab (v8.9.1) virtualized QEMU boot smoke using $($qemuExe.Name)..." -ForegroundColor Green
    ...
    if (-not ($logContent -like "*version: 8.9.1*")) {
        throw "QEMU boot smoke test failed: 'version: 8.9.1' not found in serial log!"
    }
...
Write-Host "Running DByteOS Alpha (v8.9.1) Package Smoke Tests..."
...
& .\scripts\package_release.ps1 -Version "8.9.1"
$zipFile = Join-Path $repoRoot "dbyte-v8.9.1-windows-x64.zip"
...
if ($vOut -ne "DByte 8.9.1") { throw "Package version mismatch: $vOut" }
...
try {
    ...
    Assert-Contains (Normalize-Output $pkgDashboardSnapshot) "OS Version: 8.9.1" "Package dashboard snapshot"
    ...
    Assert-Contains (Normalize-Output $pkgJourneySnapshot) "DByte 8.9.1" "Package Beta Journey Snapshot"
...
Assert-Contains (Normalize-Output $shellOut) "OS:      DByte  8.9.1" "Package shell status version"
Assert-Contains (Normalize-Output $shellOut) "version: DByte 8.9.1" "Package shell sysinfo version"
...
Assert-Equal $packagePromptDefault.Text "dbyte-shell> DByte 8.9.1`ndbyte-shell>" "Package shell prompt default snapshot"
...
Assert-Equal $packagePromptChange.Text "dbyte-shell> preference 'system.prompt' updated successfully.`ndbyteos> DByte 8.9.1`ndbyteos> preference 'system.prompt' updated successfully.`ndeadbyte> DByte 8.9.1`ndeadbyte> preferences reset to default seed state.`ndbyte-shell> DByte 8.9.1`ndbyte-shell>" "Package shell prompt preference snapshots"
