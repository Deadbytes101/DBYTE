...
if ($versionOut -ne "DByte 7.8.0") {
    throw "Version mismatch: expected 'DByte 7.8.0', got '$versionOut'"
}
...
Assert-Contains $shellBasic.Text "DByte 7.8.0" "shell version"
...
Write-Host "Running DByteOS Command Set (v7.8.0) smoke tests..."
...
==================================================
System:
  Version:    DByte  7.8.0  ( Userland Prototype )
...
try {
    ...
    # --- v7.8.0 Exact Snapshot Assertions ---
...
"@
    ...
    Assert-Equal $dbyteosPromptDefault.Text "dbyte-shell> DByte 7.8.0`ndbyte-shell>" "dbyteos shell prompt default snapshot"
    ...
    Assert-Equal $dbyteosPromptChange.Text "dbyte-shell> preference 'system.prompt' updated successfully.`ndbyteos> DByte 7.8.0`ndbyteos> preference 'system.prompt' updated successfully.`ndeadbyte> DByte 7.8.0`ndeadbyte> preferences reset to default seed state.`ndbyte-shell> DByte 7.8.0`ndbyte-shell>" "dbyteos shell prompt preference snapshots"
    ...
    Assert-Contains $dbyteosSysinfoRoot.Text "version: DByte 7.8.0" "dbyteos sysinfo version"
    ...
    Assert-Contains $dbyteosProfileRoot.Text "os_version: 7.8.0" "dbyteos profile os version"
    ...
    Write-Host "Running DByteOS Security/Permissions (v7.8.0) smoke tests..."
    ...
    Assert-Contains $dbyteosReadEtc.Text "pub let os_version: str = `"7.8.0`"" "read etc allowed"
    ...
    Write-Host "Running DByteOS Security Enforcement Expansion (v7.8.0) smoke tests..."
    $enforcementInput = @"
clean
cat etc/system.dby
...
"@
    ...
    Assert-Contains $dbyteosEnforcement.Text "os_version: str = `"7.8.0`"" "cat etc allowed"
    ...
    Write-Host "Running DByteOS Security Enforcement Hardening (v7.8.0) smoke tests..."
    $hardeningInput = @"
clean
cat boot.dby
touch boot.dby
...
"@
    ...
    Write-Host "Verifying DByteOS Personal Workspace Beta Foundation (v7.8.0) documentation..."
    ...
    Assert-Contains $mainReadme "DByteOS Personal Workspace Beta Foundation (v7.8.0)" "README Personal Workspace Beta Foundation positioning"
    ...
    Assert-Contains $osReadme "DByteOS Personal Workspace Beta Foundation (v7.8.0)" "OS README Personal Workspace Beta Foundation positioning"
    ...
    Assert-Contains $configGuide "read-only in v7.8.0" "config guide read-only"
    ...
    Assert-Contains $snapshotGuide "read-only in v7.8.0" "snapshot guide read-only"
    ...
    Assert-Contains $projectsGuide "v7.8.0 hardening" "projects guide hardening"
    ...
    # v7.8.0 enforcement confirmed via smoke tests above
    ...
    Assert-Contains $dbyteosCmdShell.Text "version: DByte 7.8.0" "dbyteos shell sysinfo"
    ...
    Assert-Contains $dbyteosCmdShell.Text "os_version: 7.8.0" "dbyteos shell profile"
    ...
    Write-Host "Running DByteOS Notes Workflow (v7.8.0) smoke tests..."
    ...
    Write-Host "Running DByteOS Notes Hardening (v7.8.0) smoke tests..."
    $notesInput = @"
clean
notes read
notes add
...
"@
    ...
    Write-Host "Running DByteOS Init Services (v7.8.0) smoke tests..."
    ...
    Write-Host "Running DByteOS Journal/Logger (v7.8.0) smoke tests..."
...
"@
    ...
    Write-Host "Running DByteOS Search (v7.8.0) smoke tests..."
    ...
    # --- v7.8.0 Deterministic Rejections ---
    ...
    # --- v7.8.0 Search UX Missing Cache / Scan Fallback tests ---
    ...
    # --- v7.8.0 Search UX Active Cache tests ---
...
"@
    ...
    # --- DByteOS Workspace Dashboard (v7.8.0) tests ---
...
Write-Host "Verifying DByteOS Kernel Lab (v7.8.0) freestanding build..."
...
Write-Host "Verifying DByteOS Kernel Lab (v7.8.0) exception status UX contracts..."
...
Assert-Contains $kernelInterruptDocs 'Exact bit set tracked for v7.8.0: `P / W/R / U/S / RSVD / I/D`.' "interrupt docs exact error-code bit names"
...
if (-not $qemuExe) {
    ...
    Write-Host "Verifying DByteOS Kernel Lab (v7.8.0) virtualized QEMU boot smoke using $($qemuExe.Name)..." -ForegroundColor Green
    ...
    if (-not ($logContent -like "*version: 7.8.0*")) {
        throw "QEMU boot smoke test failed: 'version: 7.8.0' not found in serial log!"
    }
...
Write-Host "Running DByteOS Alpha (v7.8.0) Package Smoke Tests..."
...
& .\scripts\package_release.ps1 -Version "7.8.0"
$zipFile = Join-Path $repoRoot "dbyte-v7.8.0-windows-x64.zip"
...
if ($vOut -ne "DByte 7.8.0") { throw "Package version mismatch: $vOut" }
...
try {
    ...
    Assert-Contains (Normalize-Output $pkgDashboardSnapshot) "OS Version: 7.8.0" "Package dashboard snapshot"
    ...
    Assert-Contains (Normalize-Output $pkgJourneySnapshot) "DByte 7.8.0" "Package Beta Journey Snapshot"
...
Assert-Contains (Normalize-Output $shellOut) "OS:      DByte  7.8.0" "Package shell status version"
Assert-Contains (Normalize-Output $shellOut) "version: DByte 7.8.0" "Package shell sysinfo version"
...
Assert-Equal $packagePromptDefault.Text "dbyte-shell> DByte 7.8.0`ndbyte-shell>" "Package shell prompt default snapshot"
...
Assert-Equal $packagePromptChange.Text "dbyte-shell> preference 'system.prompt' updated successfully.`ndbyteos> DByte 7.8.0`ndbyteos> preference 'system.prompt' updated successfully.`ndeadbyte> DByte 7.8.0`ndeadbyte> preferences reset to default seed state.`ndbyte-shell> DByte 7.8.0`ndbyte-shell>" "Package shell prompt preference snapshots"
