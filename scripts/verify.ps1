...
if ($versionOutput -ne 'DByte 9.0.0') {
    throw "Version mismatch: expected 'DByte 9.0.0', got '$versionOutput'"
}
...
Assert-Contains $shellBasic.Text "DByte 9.0.0" "shell version"
...
Write-Host "Running DByteOS Command Set (v9.0.0) smoke tests..."
...
==================================================
System:
  Version:    DByte  9.0.0  ( Userland Prototype )
...
try {
    ...
    # --- v9.0.0 Exact Snapshot Assertions ---
...
"@
    ...
    Assert-Equal $dbyteosPromptDefault.Text "dbyte-shell> DByte 9.0.0`ndbyte-shell>" "dbyteos shell prompt default snapshot"
    ...
    Assert-Equal $dbyteosPromptChange.Text "dbyte-shell> preference 'system.prompt' updated successfully.`ndbyteos> DByte 9.0.0`ndbyteos> preference 'system.prompt' updated successfully.`ndeadbyte> DByte 9.0.0`ndeadbyte> preferences reset to default seed state.`ndbyte-shell> DByte 9.0.0`ndbyte-shell>" "dbyteos shell prompt preference snapshots"
    ...
    Assert-Contains $dbyteosSysinfoRoot.Text "version: DByte 9.0.0" "dbyteos sysinfo version"
    ...
    Assert-Contains $dbyteosProfileRoot.Text "os_version: 9.0.0" "dbyteos profile os version"
    ...
    Write-Host "Running DByteOS Security/Permissions (v9.0.0) smoke tests..."
    ...
    Assert-Contains $dbyteosReadEtc.Text "pub let os_version: str = `"9.0.0`"" "read etc allowed"
    ...
    Write-Host "Running DByteOS Security Enforcement Expansion (v9.0.0) smoke tests..."
    $enforcementInput = @"
clean
cat etc/system.dby
...
"@
    ...
    Assert-Contains $dbyteosEnforcement.Text "os_version: str = `"9.0.0`"" "cat etc allowed"
    ...
    Write-Host "Running DByteOS Security Enforcement Hardening (v9.0.0) smoke tests..."
    $hardeningInput = @"
clean
cat boot.dby
touch boot.dby
...
"@
    ...
    Write-Host "Verifying DByteOS Personal Workspace Beta Foundation (v9.0.0) documentation..."
    ...
    Assert-Contains $mainReadme "DByteOS Personal Workspace Beta Foundation (v9.0.0)" "README Personal Workspace Beta Foundation positioning"
    ...
    Assert-Contains $osReadme "DByteOS Personal Workspace Beta Foundation (v9.0.0)" "OS README Personal Workspace Beta Foundation positioning"
    ...
    Assert-Contains $configGuide "read-only in v9.0.0" "config guide read-only"
    ...
    Assert-Contains $snapshotGuide "read-only in v9.0.0" "snapshot guide read-only"
    ...
    Assert-Contains $projectsGuide "v9.0.0 disabled path foundation" "projects guide hardening"
    ...
    # v9.0.0 enforcement confirmed via smoke tests above
    ...
    Assert-Contains $dbyteosCmdShell.Text "version: DByte 9.0.0" "dbyteos shell sysinfo"
    ...
    Assert-Contains $dbyteosCmdShell.Text "os_version: 9.0.0" "dbyteos shell profile"
    ...
    Write-Host "Running DByteOS Notes Workflow (v9.0.0) smoke tests..."
    ...
    Write-Host "Running DByteOS Notes Hardening (v9.0.0) smoke tests..."
    $notesInput = @"
clean
notes read
notes add
...
"@
    ...
    Write-Host "Running DByteOS Init Services (v9.0.0) smoke tests..."
    ...
    Write-Host "Running DByteOS Journal/Logger (v9.0.0) smoke tests..."
...
"@
    ...
    Write-Host "Running DByteOS Search (v9.0.0) smoke tests..."
    ...
    # --- v9.0.0 Deterministic Rejections ---
    ...
    # --- v9.0.0 Search UX Missing Cache / Scan Fallback tests ---
    ...
    # --- v9.0.0 Search UX Active Cache tests ---
...
"@
    ...
    # --- DByteOS Workspace Dashboard (v9.0.0) tests ---
...
Write-Host "Verifying DByteOS Kernel Lab (v9.0.0) freestanding build..."
...
Write-Host "Verifying DByteOS Kernel Lab (v9.0.0) exception status UX contracts..."
...
Assert-Contains $kernelPicSource "v9.0.0 keeps read-only PIC remap state telemetry" "pic source state telemetry milestone wording"
...
Assert-Contains $kernelInterruptDocs 'Exact bit set tracked for v9.0.0: `P / W/R / U/S / RSVD / I/D`.' "interrupt docs exact error-code bit names"
...
Assert-Contains $kernelIrqDocs '**IRQ0 timer**: skeleton planned PIT timer interrupt; bind smoke stub is dormant in `v9.0.0`.' "irq docs irq0 skeleton dormant"
Assert-Contains $kernelIrqDocs '**IRQ1 keyboard**: skeleton planned PS/2 keyboard interrupt; bind smoke stub is dormant in `v9.0.0`.' "irq docs irq1 skeleton dormant"
...
# ==============================================================================
# DByteOS Kernel Lab Hardening Verify Guards (v9.0.0)
# ==============================================================================
Write-Host "Verifying DByteOS Kernel Lab (v9.0.0) hardening guards..." -ForegroundColor Green
...
if (-not $qemuExe) {
    ...
    Write-Host "Verifying DByteOS Kernel Lab (v9.0.0) virtualized QEMU boot smoke using $($qemuExe.Name)..." -ForegroundColor Green
    ...
    if (-not ($logContent -like "*version: 9.0.0*")) {
        throw "QEMU boot smoke test failed: 'version: 9.0.0' not found in serial log!"
    }
...
Write-Host "Running DByteOS Alpha (v9.0.0) Package Smoke Tests..."
...
& .\scripts\package_release.ps1 -Version "9.0.0"
$zipFile = Join-Path $repoRoot "dbyte-v9.0.0-windows-x64.zip"
...
if ($vOut -ne "DByte 9.0.0") { throw "Package version mismatch: $vOut" }
...
try {
    ...
    Assert-Contains (Normalize-Output $pkgDashboardSnapshot) "OS Version: 9.0.0" "Package dashboard snapshot"
    ...
    Assert-Contains (Normalize-Output $pkgJourneySnapshot) "DByte 9.0.0" "Package Beta Journey Snapshot"
...
Assert-Contains (Normalize-Output $shellOut) "OS:      DByte  9.0.0" "Package shell status version"
Assert-Contains (Normalize-Output $shellOut) "version: DByte 9.0.0" "Package shell sysinfo version"
...
Assert-Equal $packagePromptDefault.Text "dbyte-shell> DByte 9.0.0`ndbyte-shell>" "Package shell prompt default snapshot"
...
Assert-Equal $packagePromptChange.Text "dbyte-shell> preference 'system.prompt' updated successfully.`ndbyteos> DByte 9.0.0`ndbyteos> preference 'system.prompt' updated successfully.`ndeadbyte> DByte 9.0.0`ndeadbyte> preferences reset to default seed state.`ndbyte-shell> DByte 9.0.0`ndbyte-shell>" "Package shell prompt preference snapshots"
