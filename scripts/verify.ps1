...
if ($versionOut -ne "DByte 8.11.2") {
    throw "Version mismatch: expected 'DByte 8.11.2', got '$versionOut'"
}
...
Assert-Contains $shellBasic.Text "DByte 8.11.2" "shell version"
...
Write-Host "Running DByteOS Command Set (v8.11.2) smoke tests..."
...
==================================================
System:
  Version:    DByte  8.11.2  ( Userland Prototype )
...
try {
    ...
    # --- v8.11.2 Exact Snapshot Assertions ---
...
"@
    ...
    Assert-Equal $dbyteosPromptDefault.Text "dbyte-shell> DByte 8.11.2`ndbyte-shell>" "dbyteos shell prompt default snapshot"
    ...
    Assert-Equal $dbyteosPromptChange.Text "dbyte-shell> preference 'system.prompt' updated successfully.`ndbyteos> DByte 8.11.2`ndbyteos> preference 'system.prompt' updated successfully.`ndeadbyte> DByte 8.11.2`ndeadbyte> preferences reset to default seed state.`ndbyte-shell> DByte 8.11.2`ndbyte-shell>" "dbyteos shell prompt preference snapshots"
    ...
    Assert-Contains $dbyteosSysinfoRoot.Text "version: DByte 8.11.2" "dbyteos sysinfo version"
    ...
    Assert-Contains $dbyteosProfileRoot.Text "os_version: 8.11.2" "dbyteos profile os version"
    ...
    Write-Host "Running DByteOS Security/Permissions (v8.11.2) smoke tests..."
    ...
    Assert-Contains $dbyteosReadEtc.Text "pub let os_version: str = `"8.11.2`"" "read etc allowed"
    ...
    Write-Host "Running DByteOS Security Enforcement Expansion (v8.11.2) smoke tests..."
    $enforcementInput = @"
clean
cat etc/system.dby
...
"@
    ...
    Assert-Contains $dbyteosEnforcement.Text "os_version: str = `"8.11.2`"" "cat etc allowed"
    ...
    Write-Host "Running DByteOS Security Enforcement Hardening (v8.11.2) smoke tests..."
    $hardeningInput = @"
clean
cat boot.dby
touch boot.dby
...
"@
    ...
    Write-Host "Verifying DByteOS Personal Workspace Beta Foundation (v8.11.2) documentation..."
    ...
    Assert-Contains $mainReadme "DByteOS Personal Workspace Beta Foundation (v8.11.2)" "README Personal Workspace Beta Foundation positioning"
    ...
    Assert-Contains $osReadme "DByteOS Personal Workspace Beta Foundation (v8.11.2)" "OS README Personal Workspace Beta Foundation positioning"
    ...
    Assert-Contains $configGuide "read-only in v8.11.2" "config guide read-only"
    ...
    Assert-Contains $snapshotGuide "read-only in v8.11.2" "snapshot guide read-only"
    ...
    Assert-Contains $projectsGuide "v8.11.2 disabled path foundation" "projects guide hardening"
    ...
    # v8.11.2 enforcement confirmed via smoke tests above
    ...
    Assert-Contains $dbyteosCmdShell.Text "version: DByte 8.11.2" "dbyteos shell sysinfo"
    ...
    Assert-Contains $dbyteosCmdShell.Text "os_version: 8.11.2" "dbyteos shell profile"
    ...
    Write-Host "Running DByteOS Notes Workflow (v8.11.2) smoke tests..."
    ...
    Write-Host "Running DByteOS Notes Hardening (v8.11.2) smoke tests..."
    $notesInput = @"
clean
notes read
notes add
...
"@
    ...
    Write-Host "Running DByteOS Init Services (v8.11.2) smoke tests..."
    ...
    Write-Host "Running DByteOS Journal/Logger (v8.11.2) smoke tests..."
...
"@
    ...
    Write-Host "Running DByteOS Search (v8.11.2) smoke tests..."
    ...
    # --- v8.11.2 Deterministic Rejections ---
    ...
    # --- v8.11.2 Search UX Missing Cache / Scan Fallback tests ---
    ...
    # --- v8.11.2 Search UX Active Cache tests ---
    ...
    # --- DByteOS Timeline (v8.11.2) tests ---
...
"@
    ...
    # --- DByteOS Workspace Dashboard (v8.11.2) tests ---
...
Write-Host "Verifying DByteOS Kernel Lab (v8.11.2) freestanding build..."
...
Write-Host "Verifying DByteOS Kernel Lab (v8.11.2) exception status UX contracts..."
...
if ($picRemapStateCalls -ne 2 -or $picRemapHistoryCalls -ne 1 -or $picRemapPreflightCalls -ne 1) {
    throw "Kernel PIC remap telemetry guard failed: expected state=2 (command + system), history=1, preflight=1; found state=$picRemapStateCalls history=$picRemapHistoryCalls preflight=$picRemapPreflightCalls"
}
...
foreach ($call in [regex]::Matches($kernelMainSource, 'pic::ProgrammableInterruptController::pic_remap_state\(\)')) {
    ...
    if (-not ($nearStateCommand -or $nearSystemCommand)) {
        throw "Kernel PIC remap telemetry guard failed: pic_remap_state() call outside pic-remap-state/system dispatch"
    }
}
...
Assert-Contains $kernelPicSource "v8.11.2 hardens read-only PIC remap state telemetry" "pic source state telemetry hardening milestone wording"
...
Assert-Contains $kernelInterruptDocs "Keyboard Symbol Decode Hotfix" "interrupt docs mark keyboard symbol decode hotfix release"
...
Assert-Contains $kernelInterruptDocs 'Exact bit set tracked for v8.11.2: `P / W/R / U/S / RSVD / I/D`.' "interrupt docs exact error-code bit names"
...
Assert-Contains $kernelIrqDocs "keyboard symbol decode hotfix" "irq docs keyboard symbol decode hotfix release"
Assert-Contains $kernelBootSmokeDocs "Keyboard Symbol Decode Hotfix release" "qemu docs keyboard symbol decode hotfix release"
...
Assert-Contains $kernelIrqDocs "PIC/IRQ remains blocked for runtime use" "irq docs runtime irq blocked"
...
Assert-Contains $kernelIrqDocs '**IRQ0 timer**: skeleton planned PIT timer interrupt; disabled in `v8.11.2`.' "irq docs irq0 skeleton disabled"
Assert-Contains $kernelIrqDocs '**IRQ1 keyboard**: skeleton planned PS/2 keyboard interrupt; disabled in `v8.11.2`.' "irq docs irq1 skeleton disabled"
...
Assert-Contains $kernelBootSmokeDocs "commands: help about version clear echo mem uptime banner keyboard reboot-note system cls status mods keys prompt int3 div0 exception exception-reset handlers handlers --active exception-status exceptions exceptions --verbose exception-help exception-about fault-status fault-reset pf-note pf-status pf-smoke irq-note irq-status irq-handlers eoi-note eoi-status irq-gates irq-gate-status irq-gate-plan irq-bind-note irq-bind-status irq-readiness irq-risk irq-preflight pic-note pic-status pic-plan pic-remap-arm pic-remap-smoke pic-remap-status pic-remap-state pic-remap-history pic-remap-preflight irq-map pic-status --verbose" "qemu docs help snapshot"
...
# ==============================================================================
# DByteOS Kernel Lab Hardening Verify Guards (v8.11.2)
# ==============================================================================
Write-Host "Verifying DByteOS Kernel Lab (v8.11.2) hardening guards..." -ForegroundColor Green
...
if (-not $qemuExe) {
    ...
    Write-Host "Verifying DByteOS Kernel Lab (v8.11.2) virtualized QEMU boot smoke using $($qemuExe.Name)..." -ForegroundColor Green
    ...
    if (-not ($logContent -like "*version: 8.11.2*")) {
        throw "QEMU boot smoke test failed: 'version: 8.11.2' not found in serial log!"
    }
...
Write-Host "Running DByteOS Alpha (v8.11.2) Package Smoke Tests..."
...
& .\scripts\package_release.ps1 -Version "8.11.2"
$zipFile = Join-Path $repoRoot "dbyte-v8.11.2-windows-x64.zip"
...
if ($vOut -ne "DByte 8.11.2") { throw "Package version mismatch: $vOut" }
...
try {
    ...
    Assert-Contains (Normalize-Output $pkgDashboardSnapshot) "OS Version: 8.11.2" "Package dashboard snapshot"
    ...
    Assert-Contains (Normalize-Output $pkgJourneySnapshot) "DByte 8.11.2" "Package Beta Journey Snapshot"
...
Assert-Contains (Normalize-Output $shellOut) "OS:      DByte  8.11.2" "Package shell status version"
Assert-Contains (Normalize-Output $shellOut) "version: DByte 8.11.2" "Package shell sysinfo version"
...
Assert-Equal $packagePromptDefault.Text "dbyte-shell> DByte 8.11.2`ndbyte-shell>" "Package shell prompt default snapshot"
...
Assert-Equal $packagePromptChange.Text "dbyte-shell> preference 'system.prompt' updated successfully.`ndbyteos> DByte 8.11.2`ndbyteos> preference 'system.prompt' updated successfully.`ndeadbyte> DByte 8.11.2`ndeadbyte> preferences reset to default seed state.`ndbyte-shell> DByte 8.11.2`ndbyte-shell>" "Package shell prompt preference snapshots"
