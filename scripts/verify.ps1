...
if ($versionOut -ne "DByte 4.0.0") {
    throw "Version mismatch: expected 'DByte 4.0.0', got '$versionOut'"
}
...
Assert-Contains $shellBasic.Text "DByte 4.0.0" "shell version"
...
Write-Host "Running DByteOS Command Set (v4.0.0) smoke tests..."
...
try {
    ...
    Assert-Contains $dbyteosSysinfoRoot.Text "version: DByte 4.0.0" "dbyteos sysinfo version"
    ...
    Assert-Contains $dbyteosProfileRoot.Text "os_version: 4.0.0" "dbyteos profile os version"
    ...
    Write-Host "Running DByteOS Security/Permissions (v4.0.0) smoke tests..."
    ...
    Assert-Contains $dbyteosReadEtc.Text "pub let os_version: str = `"4.0.0`"" "read etc allowed"
    ...
    Write-Host "Running DByteOS Security Enforcement Expansion (v4.0.0) smoke tests..."
    $enforcementInput = @"
clean
cat etc/system.dby
...
"@
    ...
    Assert-Contains $dbyteosEnforcement.Text "os_version: str = `"4.0.0`"" "cat etc allowed"
    ...
    Write-Host "Running DByteOS Security Enforcement Hardening (v4.0.0) smoke tests..."
    $hardeningInput = @"
clean
cat boot.dby
touch boot.dby
...
"@
    ...
    Write-Host "Verifying DByteOS Alpha Userland (v4.0.0) documentation..."
    ...
    Assert-Contains $mainReadme "DByteOS Alpha Userland (v4.0.0)" "README alpha positioning"
    ...
    Assert-Contains $osReadme "DByteOS Alpha Userland (v4.0.0)" "OS README alpha positioning"
    ...
    # v4.0.0 enforcement confirmed via smoke tests above
    ...
    Assert-Contains $dbyteosCmdShell.Text "version: DByte 4.0.0" "dbyteos shell sysinfo"
    ...
    Assert-Contains $dbyteosCmdShell.Text "os_version: 4.0.0" "dbyteos shell profile"
    ...
    Write-Host "Running DByteOS Notes Workflow (v4.0.0) smoke tests..."
    ...
    Write-Host "Running DByteOS Notes Hardening (v4.0.0) smoke tests..."
    $notesInput = @"
clean
notes read
notes add
...
"@
    ...
    Write-Host "Running DByteOS Init Services (v4.0.0) smoke tests..."
    ...
    Write-Host "Running DByteOS Journal/Logger (v4.0.0) smoke tests..."
