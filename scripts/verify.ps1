...
if ($versionOut -ne "DByte 7.7.1") {
    throw "Version mismatch: expected 'DByte 7.7.1', got '$versionOut'"
}
...
Assert-Contains $shellBasic.Text "DByte 7.7.1" "shell version"
...
Write-Host "Running DByteOS Command Set (v7.7.1) smoke tests..."
...
==================================================
System:
  Version:    DByte  7.7.1  ( Userland Prototype )
...
try {
    ...
    # --- v7.7.1 Exact Snapshot Assertions ---
...
"@
    ...
    Assert-Equal $dbyteosPromptDefault.Text "dbyte-shell> DByte 7.7.1`ndbyte-shell>" "dbyteos shell prompt default snapshot"
    ...
    Assert-Equal $dbyteosPromptChange.Text "dbyte-shell> preference 'system.prompt' updated successfully.`ndbyteos> DByte 7.7.1`ndbyteos> preference 'system.prompt' updated successfully.`ndeadbyte> DByte 7.7.1`ndeadbyte> preferences reset to default seed state.`ndbyte-shell> DByte 7.7.1`ndbyte-shell>" "dbyteos shell prompt preference snapshots"
    ...
    Assert-Contains $dbyteosSysinfoRoot.Text "version: DByte 7.7.1" "dbyteos sysinfo version"
    ...
    Assert-Contains $dbyteosProfileRoot.Text "os_version: 7.7.1" "dbyteos profile os version"
    ...
    Write-Host "Running DByteOS Security/Permissions (v7.7.1) smoke tests..."
    ...
    Assert-Contains $dbyteosReadEtc.Text "pub let os_version: str = `"7.7.1`"" "read etc allowed"
    ...
    Write-Host "Running DByteOS Security Enforcement Expansion (v7.7.1) smoke tests..."
    $enforcementInput = @"
clean
cat etc/system.dby
...
"@
    ...
    Assert-Contains $dbyteosEnforcement.Text "os_version: str = `"7.7.1`"" "cat etc allowed"
    ...
    Write-Host "Running DByteOS Security Enforcement Hardening (v7.7.1) smoke tests..."
    $hardeningInput = @"
clean
cat boot.dby
touch boot.dby
...
"@
    ...
    Write-Host "Verifying DByteOS Personal Workspace Beta Foundation (v7.7.1) documentation..."
    ...
    Assert-Contains $mainReadme "DByteOS Personal Workspace Beta Foundation (v7.7.1)" "README Personal Workspace Beta Foundation positioning"
    ...
    Assert-Contains $osReadme "DByteOS Personal Workspace Beta Foundation (v7.7.1)" "OS README Personal Workspace Beta Foundation positioning"
    ...
    Assert-Contains $configGuide "read-only in v7.7.1" "config guide read-only"
    ...
    Assert-Contains $snapshotGuide "read-only in v7.7.1" "snapshot guide read-only"
    ...
    Assert-Contains $projectsGuide "v7.7.1 hardening" "projects guide hardening"
    ...
    # v7.7.1 enforcement confirmed via smoke tests above
    ...
    Assert-Contains $dbyteosCmdShell.Text "version: DByte 7.7.1" "dbyteos shell sysinfo"
    ...
    Assert-Contains $dbyteosCmdShell.Text "os_version: 7.7.1" "dbyteos shell profile"
    ...
    Write-Host "Running DByteOS Notes Workflow (v7.7.1) smoke tests..."
    ...
    Write-Host "Running DByteOS Notes Hardening (v7.7.1) smoke tests..."
    $notesInput = @"
clean
notes read
notes add
...
"@
    ...
    Write-Host "Running DByteOS Init Services (v7.7.1) smoke tests..."
    ...
    Write-Host "Running DByteOS Journal/Logger (v7.7.1) smoke tests..."
...
"@
    ...
    Write-Host "Running DByteOS Search (v7.7.1) smoke tests..."
    ...
    # --- v7.7.1 Deterministic Rejections ---
    ...
    # --- v7.7.1 Search UX Missing Cache / Scan Fallback tests ---
    ...
    # --- v7.7.1 Search UX Active Cache tests ---
...
"@
    ...
    # --- DByteOS Workspace Dashboard (v7.7.1) tests ---
...
Write-Host "Verifying DByteOS Kernel Lab (v7.7.1) freestanding build..."
...
Write-Host "Verifying DByteOS Kernel Lab (v7.7.1) exception status UX contracts..."
Assert-Contains $kernelMainSource "mod page_fault;" "kernel page fault skeleton module is compiled"
Assert-Contains $kernelMainSource "commands: help about version clear echo mem uptime banner keyboard reboot-note system cls status mods keys prompt int3 div0 exception exception-reset handlers exception-status exceptions exception-help pf-note" "kernel help lists exception UX commands"
...
Assert-Contains $kernelMainSource "page fault: planned / disabled\nvector: 14\ncr2: unavailable\nerror code: documented only\n" "kernel pf-note exact output"
Assert-Contains $kernelMainSource "interrupts: disabled" "kernel system and exception-status report interrupts disabled"
Assert-Contains $kernelMainSource "exception handlers: breakpoint, divide-by-zero" "kernel system active handler summary"
Assert-Contains $kernelMainSource "page fault handler: planned / disabled" "kernel system page fault planned status"
...
Assert-NotContains $kernelMainSource "entries[14].set_handler" "kernel vector 14 remains planned only"
...
Assert-NotContains $kernelMainSource "page_fault_handler" "kernel has no active page fault handler symbol"
...
Assert-NotContains $kernelMainSource "0 as *const" "kernel has no null pointer page fault trigger"
...
Assert-NotContains $kernelPageFaultSource "extern `"C`"" "page fault skeleton has no handler ABI entrypoint"
Assert-NotContains $kernelPageFaultSource "global_asm" "page fault skeleton has no assembly stub"
Assert-NotContains $kernelPageFaultSource "asm!" "page fault skeleton has no runtime CR2 read"
Assert-NotContains $kernelPageFaultSource "page_fault_handler" "page fault skeleton has no active handler symbol"
Assert-NotContains $kernelPageFaultSource "page_fault_handler_asm" "page fault skeleton has no assembly handler symbol"
Assert-NotContains $kernelPageFaultSource "page_fault_handler_rust" "page fault skeleton has no Rust handler symbol"
...
Assert-NotContains $kernelInterruptSource "page_fault" "page fault handler remains absent"
Assert-Contains $kernelInterruptDocs '| `14` | Fault | Page Fault | *Planned* | Unhandled in current version (v7.7.1). |' "interrupt docs page fault planned"
Assert-Contains $kernelInterruptDocs "Only Vector 0 and Vector 3 Handlers are Active" "interrupt docs active handler warning"
...
Assert-Contains $kernelInterruptDocs "No Page Fault Handler Yet" "interrupt docs page fault warning"
Assert-Contains $kernelInterruptDocs "Vector 14 is planned but unhandled" "interrupt docs vector 14 unhandled"
Assert-Contains $kernelInterruptDocs "Page Fault Direction Note (Vector 14)" "interrupt docs page fault direction section"
...
Assert-Contains $kernelInterruptDocs 'Exact bit set tracked for v7.7.1: `P / W/R / U/S / RSVD / I/D`.' "interrupt docs exact error-code bit names"
...
Assert-Contains $kernelInterruptDocs "Page fault is still unhandled" "interrupt docs page fault still unhandled warning"
Assert-Contains $kernelInterruptDocs 'No `entries[14].set_handler` binding, no `int 14` trigger, no runtime CR2 read' "interrupt docs no vector 14 binding or trigger"
Assert-Contains $kernelBootSmokeDocs "commands: help about version clear echo mem uptime banner keyboard reboot-note system cls status mods keys prompt int3 div0 exception exception-reset handlers exception-status exceptions exception-help pf-note" "qemu docs help snapshot"
...
Assert-Contains $kernelBootSmokeDocs "page fault: planned / disabled" "qemu docs pf-note output"
...
if (-not $qemuExe) {
    ...
    Write-Host "Verifying DByteOS Kernel Lab (v7.7.1) virtualized QEMU boot smoke using $($qemuExe.Name)..." -ForegroundColor Green
    ...
    if (-not ($logContent -like "*version: 7.7.1*")) {
        throw "QEMU boot smoke test failed: 'version: 7.7.1' not found in serial log!"
    }
...
Write-Host "Running DByteOS Alpha (v7.7.1) Package Smoke Tests..."
...
& .\scripts\package_release.ps1 -Version "7.7.1"
$zipFile = Join-Path $repoRoot "dbyte-v7.7.1-windows-x64.zip"
...
if ($vOut -ne "DByte 7.7.1") { throw "Package version mismatch: $vOut" }
...
try {
    ...
    Assert-Contains (Normalize-Output $pkgDashboardSnapshot) "OS Version: 7.7.1" "Package dashboard snapshot"
    ...
    Assert-Contains (Normalize-Output $pkgJourneySnapshot) "DByte 7.7.1" "Package Beta Journey Snapshot"
...
Assert-Contains (Normalize-Output $shellOut) "OS:      DByte  7.7.1" "Package shell status version"
Assert-Contains (Normalize-Output $shellOut) "version: DByte 7.7.1" "Package shell sysinfo version"
...
Assert-Equal $packagePromptDefault.Text "dbyte-shell> DByte 7.7.1`ndbyte-shell>" "Package shell prompt default snapshot"
...
Assert-Equal $packagePromptChange.Text "dbyte-shell> preference 'system.prompt' updated successfully.`ndbyteos> DByte 7.7.1`ndbyteos> preference 'system.prompt' updated successfully.`ndeadbyte> DByte 7.7.1`ndeadbyte> preferences reset to default seed state.`ndbyte-shell> DByte 7.7.1`ndbyte-shell>" "Package shell prompt preference snapshots"
